...
struct Gradient
{
    Gradient(signed char x, signed char y, signed char z) 
    : x(x), y(y), z(z) {}
...
const struct Gradient grad3[12] = {
...
unsigned char perm[512]      = {0};
unsigned char permMod12[512] = {0};
...
double dot(const Gradient& g, double x, double y) 
{
    return g.x * x + g.y * y;
}

long fastFloor(double x) 
{
